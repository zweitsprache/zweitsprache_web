...
import { deleteLernziel, updateLernziel } from '../actions'

interface LernzielRowProps {
  id: string
  workshopId: string
  text: string
}

export function LernzielRow({ id, workshopId, text }: LernzielRowProps) {
  const [editing, setEditing] = useState(false)

  const [updateState, updateAction, isUpdating] = useActionState(
    async (_prev: { error?: string } | null, formData: FormData) => {
      const result = await updateLernziel(id, workshopId, formData)
  ...
  const [deleteState, deleteAction, isDeleting] = useActionState(
    async (_prev: { error?: string } | null, _formData: FormData) => {
      return (await deleteLernziel(id, workshopId)) ?? null
    },
    null
  )
