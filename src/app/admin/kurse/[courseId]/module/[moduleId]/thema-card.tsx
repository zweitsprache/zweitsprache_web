...
import { deleteThema } from '../../../actions'
...
export function ThemaCard({ id, courseId, moduleId, title, description, lessonCount, index }: ThemaCardProps) {
  const [deleteState, deleteAction, isDeleting] = useActionState(
    async (_prev: { error?: string } | null, _formData: FormData) => {
      return (await deleteThema(id, courseId)) ?? null
    },
    null
  )
  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      <div className="mb-2 flex items-center justify-between">
        <a
          href={`/admin/kurse/${courseId}/module/${moduleId}/themen/${id}`}
